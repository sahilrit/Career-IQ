...
## 2C. AI features (bring-your-own-key)

AI cover letters use **Anthropic Claude** with **each workspace's own API
key** — CareerOS never pays for AI. Users add their key in the app under
**Settings**; with no key, generation stays on the free templates.

- `CAREEROS_SECRET_KEY` encrypts stored keys at rest. On Render it is
  auto-generated by the Blueprint (`generateValue: true`) and **must stay
  stable** — rotating or clearing it makes every stored key undecryptable
  (users would just re-enter theirs). Any string works; it is hashed into
  a Fernet key.
- `CAREEROS_AI_MODEL` (optional) overrides the default
  `claude-haiku-4-5-20251001`.

To use AI yourself: create a key at console.anthropic.com, add a little
credit, paste it into Settings. A transient AI error (bad key, quota,
timeout) never blocks generation — it falls back to the template.
